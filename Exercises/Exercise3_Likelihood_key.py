...
data = 3  # Supply observed number of successes here.
numTrials = 5
...
pVals = scipy.arange(0,1.01,0.05)
pVals = [round(p,2) for p in pVals]
print(pVals)

# Calculate the likelihood scores for these values of p, in light of the data you've collected

likes = [binomPMF(data,numTrials,p) for p in pVals]
likes = [round(like,5) for like in likes]
print(likes)

# Find the maximum likelihood value of p (at least, the max in this set)

maxPVal = round(pVals[likes.index(max(likes))],2)
print(maxPVal)

# What is the strength of evidence against the most extreme values of p (0 and 1)?

# -- Probably infinite. At least one of them must be. Their likelihoods are 0, meaning that there is no way they could have produced the observed data.

# Calculate the likelihood ratios comparing each value (in the numerator) to the max value (in the denominator)

likeRatios = [round(like/max(likes),3) for like in likes]
print(likeRatios)
...
data = 13  # Supply observed number of successes here.
numTrials = 20


# Calculate the likelihood scores for these values of p, in light of the data you've collected

likes_20 = [binomPMF(data,numTrials,p) for p in pVals]
likes_20 = [round(like,5) for like in likes_20]
print(likes_20)

# Find the maximum likelihood value of p (at least, the max in this set)

maxPVal_20 = round(pVals[likes_20.index(max(likes_20))],2)
print(maxPVal_20)
...
likeRatios_20 = [round(like/max(likes_20),3) for like in likes_20]
print(likeRatios_20)

# When is the ratio small enough to reject some values of p?

# -- You will discover this on your own later in this exercise.
...
def MLbinomOptim_while(k,n,pCurr=0.5,diff=0.1,thresh=0.001):
    """
    This function optimizes the value of p for a binomial according to ML, based on
    observed data (k successes in n trial). Uses while loops instead of recursion.
    """
    pCurrLike = binomPMF(k,n,pCurr)
    pUpLike = binomPMF(k,n,(pCurr+diff))
    pDownLike = binomPMF(k,n,(pCurr-diff))
    while (diff > thresh):
        while (pCurrLike < pUpLike) or (pCurrLike < pDownLike):
            if pUpLike > pCurrLike:
                pCurr = pCurr + diff
            elif pDownLike > pCurrLike:
                pCurr = pCurr- diff
            pCurrLike = binomPMF(k,n,pCurr)
            pUpLike = binomPMF(k,n,(pCurr+diff))
            pDownLike = binomPMF(k,n,(pCurr-diff))
...
def MLbinomOptim_recursive(k,n,pCurr=0.5,diff=0.1,thresh=0.001):
    """
    This function optimizes the value of p for a binomial according to ML, based on
    observed data (k successes in n trial). Uses recursion instead of while loops.
    """
    pCurrLike = binomPMF(k,n,pCurr)
    pUpLike = binomPMF(k,n,(pCurr+diff))
    pDownLike = binomPMF(k,n,(pCurr-diff))
    while (diff > thresh):
        if (pUpLike > pCurrLike):
            pCurr = MLbinomOptim_recursive(k,n,(pCurr+diff),diff,thresh)
        if (pDownLike > pCurrLike):
            pCurr = MLbinomOptim_recursive(k,n,(pCurr-diff),diff,thresh)
        diff *= 0.5
        pCurrLike = binomPMF(k,n,pCurr)
        pUpLike = binomPMF(k,n,(pCurr+diff))
...
for i in range(reps):
    if i % 100 == 0:
        print (i),
        sys.stdout.flush()
    sims.append(binom.rvs(trials,trueP))
...
for i in range(reps):
    if i % 100 == 0:
            print (i),
            sys.stdout.flush()
    MLests.append(MLbinomOptim_recursive(sims[i],trials))
...
for i in range(reps):
    if i % 100 == 0:
            print (i),
            sys.stdout.flush()
    LRs.append(-2*math.log(binomPMF(sims[i],trials,trueP)/binomPMF(sims[i],trials,MLests[i])))
LRs.sort()
LRs_95perc = LRs[int(math.floor(reps*0.95))]
print(LRs_95perc)
